import * as requestFromServer from "./setting.crud";
import { moduleSlice, callTypes } from "./setting.slice";
...
////use
export const changePassword = record => dispatch => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer
    .changePassword(record)
    .then(() => {
      dispatch(actions.passwordChange({ record }));
      successHandling("change password");
    })
    .catch(error => {
      error.clientMessage = "Can't update data";
      dispatch(actions.catchError({ error, callType: callTypes.action }));
